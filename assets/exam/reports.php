...
function aspire_reports_dashboard_shortcode() {
    global $wpdb;
    ...
    <div class="container-fluid">
        <div class="row">
            <?php 
            $active_section = $section; // Match sidebar naming
            ...
            <div class="col-md-9 p-4">
                ...
                switch ($section) {
                    case 'view-reports':
                        ...
                        <div class="card">
                            ...
                            <div class="card-body">
                                ...
                                <?php
                                if ($search_query) {
                                    $exams_query = $wpdb->prepare(
                                        "SELECT * FROM {$wpdb->prefix}exams WHERE education_center_id = %s AND name LIKE %s",
                                        $education_center_id,
                                ...
                                if (empty($exams)) {
                                    ...
                                    foreach ($exams as $exam) {
                                        ...
                                        echo '<td>';
                                        echo '<a href="?section=generate-reports&exam_id=' . $exam->id . '" class="btn btn-sm btn-primary me-2">Generate Report</a>';
                                        echo '<a href="?section=view-reports&exam_id=' . $exam->id . '&view_details=true" class="btn btn-sm btn-info">View Details</a>';
                                        echo '</td>';
                                        echo '</tr>';
                                    }
                                ...
                                // Detailed View Popup
                                if (isset($_GET['view_details']) && isset($_GET['exam_id'])) {
                                    $detail_exam_id = intval($_GET['exam_id']);
                                    $detail_exam = $wpdb->get_row($wpdb->prepare(
                                        "SELECT * FROM {$wpdb->prefix}exams WHERE id = %d AND education_center_id = %s",
                                        $detail_exam_id, $education_center_id
                                    ));
                                    if ($detail_exam) {
                                        ...
                                        <div class="card mt-4 border-primary">
                                            <div class="card-header bg-primary text-white">
                                                <h5 class="card-title mb-0">Detailed Report: <?php echo esc_html($detail_exam->name); ?></h5>
                                            </div>
                                            <div class="card-body">
                                                <?php
                                                $student_marks = [];
                                                ...
                                                foreach ($student_marks as $student_name => $marks) {
                                                    echo '<h6>' . esc_html($student_name) . '</h6>';
                                                    echo '<ul class="list-group mb-3">';
                                                    foreach ($marks as $subject => $mark) {
                                                        echo '<li class="list-group-item d-flex justify-content-between align-items-center">';
                                                        echo esc_html($subject) . '<span class="badge bg-primary rounded-pill">' . $mark . '</span>';
                                                        echo '</li>';
                                                    }
                                                    echo '</ul>';
                                                }
                                                ?>
                                                <a href="?section=view-reports" class="btn btn-secondary mt-3">Close</a>
                                            </div>
                                        </div>
                                        <?php
                                    }
                    ...
                    case 'generate-reports':
                        ...
                        $filtered_results = array_filter($results, function($result) use ($wpdb, $search_query) {
                            ...
                            $student_name = $student ? $student->post_title : 'Unknown (' . $result['student_id'] . ')';
                        ...
                        if (isset($_POST['generate_pdf']) && wp_verify_nonce($_POST['nonce'], 'generate_report_nonce')) {
                            ...
                            <head>
                                ...
                                <style>
                                    ...
                                    .performance-list .student {
                                        font-weight: bold;
                                        color: #444;
                                        flex: 2;
                                    }
                                    .performance-list .avg-marks {
                                        color: #1a2b5f;
                                        flex: 1;
                                        text-align: center;
                                    }
                                ...
                                <div class="container">
                                    ...
                                    <div class="report-details">
                                        <div class="detail-item">
                                            ...
                                            <span class="value">' . esc_html($exam->name) . '</span>
                                        </div>
                                        <div class="detail-item">
                                            <span class="label">Exam Date:</span>
                                            <span class="value">' . esc_html($exam->exam_date ?: 'N/A') . '</span>
                                        </div>
                                        <div class="detail-item">
                                            <span class="label">Class/Course:</span>
                                            <span class="value">' . esc_html($exam->class_id ?: 'N/A') . '</span>
                                        </div>
                                        <div class="detail-item">
                                        ...
                                        <div class="detail-item">
                                            ...
                                            <span class="value">' . count($results) . '</span>
                                        </div>
                                    </div>
                            ...
                            foreach ($filtered_results as $result) {
                                ...
                                $html .= '
                                            <li>
                                                <span class="student">' . esc_html($student_name) . '</span>
                                                <span class="avg-marks">' . round($result['avg_marks'], 2) . '</span>
                                            </li>';
                            }
                            ...
                            $html .= '
                                    ...
                                    <div class="footer">
                                        <p>This is an official report issued by ' . esc_html($institute_name) . '</p>
                                        ...
                                        <div class="generated-by">
                                            Report generated by Instituto Educational Center Management System
                                        </div>
                                    </div>
                                </div>
                            </body>
                            ...
                            try {
                                ...
                                header('Content-Disposition: attachment; filename="exam_report_' . $exam_id . '.pdf"');
                            ...
                            // Preview with card layout and search
                            ?>
                            <div class="card">
                                <div class="card-header d-flex justify-content-between align-items-center">
                                        ...
                                        <div class="col-md-6">
                                            <div class="card bg-light">
                                                <div class="card-body">
                                                    ...
                                                    <p><strong>Total Students:</strong> <?php echo count($results); ?></p>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                                ...
                                                <div class="col-md-4 mb-3">
                                                    <div class="card">
                                                        <div class="card-body">
                                                            ...
                                                            <p class="card-text">Average Marks: <span class="text-primary"><?php echo round($result['avg_marks'], 2); ?></span></p>
                                                            <a href="?section=generate-reports&exam_id=<?php echo $exam_id; ?>&student_id=<?php echo urlencode($result['student_id']); ?>&view_details=true" class="btn btn-sm btn-info">View Details</a>
                                                        </div>
                                                    </div>
                                                </div>
                                                <?php
